...
export type TypeTheme = {
  ...
  /**
   * Brand schema version — bump to force-reset themed defaults across users
   */
  brandVersion: number;
};

const CURRENT_BRAND_VERSION = 1;
...
export class Theme extends AbstractStore {
  ...
  default(): TypeTheme {
    return {
      ...
      brandVersion: CURRENT_BRAND_VERSION,
    };
  }
  ...
  clean(input: Partial): TypeTheme {
    ...
    const onCurrentBrand = input.brandVersion === CURRENT_BRAND_VERSION;
